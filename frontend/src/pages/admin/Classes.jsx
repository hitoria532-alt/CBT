...
import { Plus, Pencil, Trash2, Users, Search, Download, FileText, IdCard } from "lucide-react";
...
export default function Classes() {
  ...
  const downloadCards = async (c) => {
    try {
      const res = await api.get(`/cards/class/${c.id}/pdf`, { responseType: "blob" });
      const url = URL.createObjectURL(res.data);
      const a = document.createElement("a"); a.href = url; a.download = `kartu-peserta-${c.name}.pdf`; a.click();
      toast.success("Kartu peserta diunduh");
    } catch (e) { toast.error(apiError(e)); }
  };
            ...
            <div key={c.id} className="bg-card border border-border rounded-md p-6 hover:shadow-sm transition-shadow" data-testid={`class-${c.id}`}>
              ...
              <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <Button size="sm" variant="outline" onClick={() => downloadCards(c)} data-testid={`exam-cards-${c.id}`}>
                    <IdCard className="h-4 w-4 mr-1.5" />Kartu
                  </Button>
                </div>
              </div>
            </div>
